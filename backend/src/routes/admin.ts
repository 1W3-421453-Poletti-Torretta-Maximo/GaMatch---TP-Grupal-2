...
/** Wrapper that applies auth + admin check inside the handler. */
async function adminHandler(req: FastifyRequest, reply: FastifyReply, next: () => Promise<void>): Promise<void> {
...
export default async function adminRoutes(app: FastifyInstance) {
  ...
  app.get('/dashboard', async (_req, reply) => {
    await adminHandler(_req, reply, async () => {
      ...
      try {
        ...
        const topRated = topRatedResult.records.map((r) => r.get('userData'));
        const topLobbies = topLobbiesResult.records.map((r) => r.get('lobbyData'));
        const searchTimeslots = searchTimeslotsResult.records.map((r) => r.get('slotData'));
        const topMatches = topMatchesResult.records.map((r) => r.get('userData'));
  ...
  app.post('/promote', async (req, reply) => {
    await adminHandler(req, reply, async () => {
